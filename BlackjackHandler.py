...
class BlackjackHandler(MessageHandler.MessageHandler):
  ...
  async def onMessage(self, message):
    if message.content.startswith("!blackjack") and message.author.id not in self.usersStartingNewGame:
      ...
      embed.add_field(name="Instructions", value="Use \"!bet amount\" to make your bet and start playing or use !credits to check your balance", inline=True)
      await message.channel.send(embed=embed)

    if message.content.startswith("!bet") and message.author.id in self.usersStartingNewGame:
      if message.content == "!bet":
        return
      bet = float(message.content[4:])
      if self.userCredits.get(message.author.id) - bet < 0 or self.userCredits.get(message.author.id) <= 0:
        embed=discord.Embed(title="Balance too low", color=0x65077f)
        embed.add_field(name="Not enough credits", value="You are too poor for this bet. Consider using \"!beg\"", inline=True)
        await message.channel.send(embed=embed)
        return

      self.usersStartingNewGame.remove(message.author.id)
      self.userCredits.update({message.author.id: self.userCredits.get(message.author.id) - bet})
      bjg = Blackjack(bet)
    ...
    if message.content.startswith("!beg") and message.author.id in self.usersStartingNewGame:
      if self.userCredits.get(message.author.id) < 500.0:
        embed=discord.Embed(title="Dealers Pity", color=0x65077f)
        embed.add_field(name="The dealer takes pity on you", value="You are given 100 credits", inline=True)
        await message.channel.send(embed=embed)
        self.userCredits.update({message.author.id: self.userCredits.get(message.author.id) + 100.0})
        await self.currentCredits(message)
      else:
        embed=discord.Embed(title="Dealers Anger", color=0x65077f)
        embed.add_field(name="You annoyed the dealer", value="He takes your credits, how rude", inline=True)
        await message.channel.send(embed=embed)
        self.userCredits.update({message.author.id: 0.0})
        await self.currentCredits(message)
          
    if message.content.startswith("!hit") and message.author.id in self.userGameData:
      ...
      if outcome == 1:
        self.userCredits.update({message.author.id: self.userCredits.get(message.author.id) + 2 * bjg.getPlayerBet()})
        await self.playerVictory(message)
      elif outcome == 2:
        await self.dealerVictory(message)

    if message.content.startswith("!stand") and message.author.id in self.userGameData:
      bjg = self.userGameData.get(message.author.id)
      bjg.stand()
      await self.currentStandings(message)
      ...
      if outcome == 3:
        ...
        self.userCredits.update({message.author.id: self.userCredits.get(message.author.id) + bjg.getPlayerBet()})
      elif outcome == 2:
        await self.dealerVictory(message)
      elif outcome == 1:
        await self.playerVictory(message)
    ...
    if message.content.startswith("!double") and message.author.id in self.userGameData:
      bjg = self.userGameData.get(message.author.id)
      self.userCredits.update({message.author.id: self.userCredits.get(message.author.id) - bjg.getPlayerBet()})
      bjg.hit()
  ...
  async def playerVictory(self, message):
    embed=discord.Embed(title="Player wins!", color=0x65077f)
  ...
  async def currentCredits(self, message):
    ...
    embed.add_field(name="Your Credits", value=f"{self.userCredits.get(message.author.id):.2f}", inline=True)
    await message.channel.send(embed=embed)
  ...
  def evaluateGame(self, stayBool):
    ...
    if stayBool == False:
      ...
      if self.playerHandValue == self.dealerHandValue:
        return 3
      elif (self.playerHandValue < self.dealerHandValue and self.dealerHandValue <= 21) or self.playerHandValue > 21:
        return 2
      else:
        return 1
